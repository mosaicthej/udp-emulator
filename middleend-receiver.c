...
void *receive_thread(void *arg) {
  ...
  bool done, done2, hasproblemo, dropped, firstMsg; /* flags */
  /* args */
  ...
  while (!done && !done2) {
    ...
    if (getnameinfo((struct sockaddr *)&their_addr, addr_len, hbuf,
    ...
    len_tmp = addr_len;
    /* the first message of both endpoints are dealing with the
     * identifiers in the table, so must not drop */
    if (nMsgRecv1 == 0) {
      firstMsg = true;
      from_addr1 = (struct sockaddr *)&their_addr;
      len_addr1 = addr_len;
      nMsgRecv1++;
    ...
    } else if (nMsgRecv2 == 0 && !sameAddr((struct sockaddr *)&their_addr,
                                           addr_len, from_addr1, len_addr1)) {
      firstMsg = true;
      from_addr2 = (struct sockaddr *)&their_addr;
      len_addr2 = addr_len;
      nMsgRecv2++;
      do_getnameinfo(tmp_addr, addr_len, hbuf, sbuf);
      strncpy(nameTbl->right->portSnd, sbuf, NI_MAXSERV);
    }
    /* moving beyond the first message, now we can find which one to send to. */

  gen_rand(rnd);
  dropped = rnd < drop_prob;
  if (!firstMsg && !dropped &&
      !(done && sameAddr((struct sockaddr *)&their_addr, addr_len, from_addr1,
                         len_addr1)) &&
      !(done2 && sameAddr((struct sockaddr *)&their_addr, addr_len, from_addr2,
                          len_addr2))) {
    /* not dropped, and not from a closed buffer */
    tmp_addr = (struct sockaddr *)&their_addr;
    len_tmp = addr_len;
    /* got a message! (this is blocking via recvfrom) */
   if (!sameAddr(tmp_addr, len_tmp, from_addr1, len_addr1) &&
    ...
    if (sameAddr(Qmsg->fromAddr, Qmsg->fromAddrLen, from_addr1, len_addr1))
      ...
      do_testkill(rcvBuf, killMsg, done2);
  } else
    printf("listener: packet dropped :P\n"); /* dropped */
}
/* done main loop */
